      ...
      -- Non-TRC
        ...
        join monthly_viewership mv on (mv.deal_parent = w.deal_parent and mv.year_month_day = w.year_month_day and mv.channel_id = w.channel_id  )
        where w.quarter = 'q4' and w.year = 2021 and w.deal_parent not in (16, 25) and w.tot_hov is not null



        -- update records with calulcated share 
        update  wurl_viewership w 
        set w.share = q.share_calc
        from (
            select w.id as id, w.deal_parent,w.channel_id, w.year_month_day, w.tot_hov, mv.tot_hov, (w.tot_hov / mv.tot_hov)as share_calc from wurl_viewership w
            join monthly_viewership mv on (mv.deal_parent = w.deal_parent and mv.year_month_day = w.year_month_day and mv.channel_id = w.channel_id  )
            where w.quarter = 'q4' and w.year = 2021 and w.deal_parent not in (16, 25) and w.tot_hov is not null
        ) q
        where w.id = q.id
  ...
  SET av.calc_share = q.v_share 
  FROM (
    select a.id as id, a.total_viewership_minutes, (a.total_viewership_minutes / mv.tot_hov) as v_share,a.deal_parent, a.month from amagi_viewership a
    join monthly_viewership mv on (mv.year_month_day = a.year_month_day and a.channel_id = mv.channel_id and a.deal_parent = mv.deal_parent)
    where a.deal_parent in (23, 24, 28, 30, 31) and a.year =2021 and a.quarter = 'q4' and  mv.platform = 'amagi'  
  ) q
  WHERE av.id = q.id


  -- Amagi viewership share check (should all equal 1)
          select sum(CALC_SHARE), year_month_day , channel_id from amagi_viewership  where quarter = 'q4' and year = 2021 group by year_month_day, channel_id ,deal_parent
