      ...
      FROM (
        SELECT w.id as id, w.deal_parent, w.year_month_day,w.year, w.tot_hov, share, w.tot_hov / m.tot_hov as calced_share 
        FROM WURL_VIEWERSHIP w
      ...
      FROM (
        ...
        WHERE w.quarter = 'q4'  and w.year = 2021 and w.deal_parent in (17)
      ) t
      WHERE wv.id = t.id
      ...
      -- xumo
          select sum(share), year_month_day , channel from wurl_viewership where deal_parent = 17 and quarter = 'q4' and year = 2021 group by year_month_day, channel 
