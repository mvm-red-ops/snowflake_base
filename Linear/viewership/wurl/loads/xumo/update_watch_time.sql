...
SET w.tot_hov = t.tot_hours_watched
FROM  (
      SELECT title, month, ref_id , TOTAL_TIME_WATCHED, sum(TRY_CAST(SPLIT_PART(TOTAL_TIME_WATCHED, ':',1)as INTEGER)*60) as hours,
      sum(TRY_CAST(SPLIT_PART(TOTAL_TIME_WATCHED, ':',2)as INTEGER)) as mins, 
      sum(TRY_CAST(SPLIT_PART(TOTAL_TIME_WATCHED, ':',3)as INTEGER) / 60) as secs,
      hours + mins + secs as tot_hours_watched
      FROM WURL_VIEWERSHIP
      WHERE deal_parent = 17 and quarter = 'q2'  
      GROUP BY title, month, ref_id,TOTAL_TIME_WATCHED
