...
-- weighted_composite_score
    ...
    where a.series_id = wv.series_id and  quarter = 'q1' and year = 2022  and deal_parent = 18  and WEIGHTED_COMPOSITE_SCORE > .8 
    ...
    from (
      ...
      ((v3*1)+(v4*2))/3 weighted_composite_score, wv.id as id, a.ref_id as ref_id, a.content_provider as content_provider
      from wurl_viewership wv
      cross join dictionary.public.assets  a
      where  quarter = 'q1' and year = 2022  and deal_parent = 18  and WEIGHTED_COMPOSITE_SCORE > .7 and wv.series_id = a.series_id
    ) q 
    where wv.id = q.id

    -- wurl
    select channel, wv.title, tot_hov, month, wv.ref_id FROM wurl_viewership wv
    where quarter = 'q1'  and year = 2022 and (wv.ref_id = 'externalId' or wv.ref_id is null)
    ...
    from (
        ...
        where a.series_id = wv.series_id and  quarter = 'q1' and year = 2022  and (pv.ref_id = 'externalId' or pv.ref_id is null) and (weighted_composite_score > 0.7 or v3> .75 )
    ) q 
    where pv.id = q.qid
-- dictionary table 
    ...
    from pluto_viewership pv
    cross join dictionary.public.assets  a
    where a.series_id = wv.series_id and  quarter = 'q1' and year = 2022  and (pv.ref_id = 'externalId' or pv.ref_id is null) and weighted_composite_score > .7 and v3> .4
...
where (ref_id = 'externalId' or ref_id is null) and  quarter = 'q1' and year = 2022  and length(title)> 0
-- SPLIT PART WHERE CLIP NAME CONTAINS  ":"  (QUERY NOT HERE)
            ...
            from (
                ...
                from pluto_viewership pv
                cross join dictionary.public.assets  a
                where a.series_id = wv.series_id and  quarter = 'q1' and year = 2022  and (pv.ref_id = 'externalId' or pv.ref_id is null) and weighted_composite_score > 0.7
            ) q 
            where pv.id = q.qid
            ...
            -- update pluto_viewership pv
                -- will be unmatched records, will have to find title and figure out solutions for each manually, usually involves reformatting title and or adding new assets to tables
                    ...
                    --     from pluto_viewership pv
                    --     cross join dictionary.public.assets  a
                    --     where quarter = 'q1' and year = 2022  and (pv.ref_id = 'externalId' or pv.ref_id is null) 
...
-- check if anything is msising ref_id
    ...
    where (ref_id = 'externalId' or ref_id is null) and  quarter = 'q1' and year = 2022  
...
-- Amagi (SOME IF NOT ALL REF IDS SHOULD BE IN CONTENT ID COLUMN ALREADY)
    ...
    cross join dictionary.public.assets  a
    where a.series_id = av.series_id and  quarter = 'q1' and year = 2022  and deal_parent = 18  and WEIGHTED_COMPOSITE_SCORE > .8 and a.content_provider != average_watch_time_seconds.content_provider
    ...
    set a.ref_id = q.ref_id , a.content_provider = q.cp
        ...
        select av.id as id, a.content_provider as cp, av.formatted_title, a.title, soundex(av.formatted_title) t1, soundex(a.title) t2, fuzzy_score(a.title, av.formatted_title) v3, 
            ...
            cross join assets.public.metadata  a
            where a.series_id = av.series_id and  quarter = 'q1' and year = 2022 and WEIGHTED_COMPOSITE_SCORE > .8
            limit 2000
        ) q 
    where a.id = q.id
